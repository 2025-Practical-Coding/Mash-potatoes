...
def ending(gs: GameState):
    ...
    if len(gs.allies) < gs.ally_threshold:
        msg = (
            ...
            "아래의 수치에 따른 실패 정도 **참고용**이며 절대적인 답이 아닙니다\n"
            "11미만 7이상 : 동료가 조금 부족해 근소한 차이로 실패\n"
            "7미만 4이상 : 동료가 절반 정도가 없어  실패\n"
            "4미만 : 기본적인 구성도 할 수 없어 시도도 못하고 실패\n"
            "아래 **반드시** JSON 코드블록(Triple backticks)으로만 응답하세요:\n"
            "```json\n"
            "{\n"
    ...
    # 동료간 관계 확인
        ...
        msg = (
            ...
            "이에 맞춰서 바론 도전 결과와 그 이후 마무리 텍스트를 생성하세요.\n"
            "몇몇 동료들 간의 서로 사이가 좋지 않은 관계로 실패했다는 것을 명심하세요.\n"
            f"최종 동료들 간의 관계 수치는 {gs.total_relationship}입니다.\n"
            "아래의 수치에 따른 실패 정도는 **참고용**이며 절대적인 답이 아닙니다.\n"
            "10미만 5이상 : 몇몇의 동료들이 합이 안좋아 실패\n"
            "5미만 1이상 : 몇몇 동료가 서로 싸우면서 실패\n"
            "1미만 : 모든 동료들이 서로 싸우면서 실패\n" 
            "아래 **반드시** JSON 코드블록(Triple backticks)으로만 응답하세요:\n"
            "```json\n"
            "{\n"
            "  \"narration\": \"<게임 결과 설명 텍스트>\"\n"
